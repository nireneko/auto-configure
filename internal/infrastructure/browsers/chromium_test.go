...
func TestChromiumInstaller_IsInstalled_True(t *testing.T) {
	m := &mocks.MockExecutor{}
	m.AddResponse("/usr/bin/chromium", "", nil)
	inst := browsers.NewChromiumInstaller(m)
...
func TestChromiumInstaller_IsInstalled_False(t *testing.T) {
	m := &mocks.MockExecutor{}
...
func TestChromiumInstaller_Install_HappyPath(t *testing.T) {
	m := &mocks.MockExecutor{}
	m.AddResponse("", "", nil)
	m.AddResponse("", "", nil)
	inst := browsers.NewChromiumInstaller(m)
	...
	assertCall(t, m.Calls[0], "apt", "update")
	assertCall(t, m.Calls[1], "apt", "install", "-y", "chromium")
}

func TestChromiumInstaller_Install_AptLock(t *testing.T) {
	m := &mocks.MockExecutor{}
	m.AddResponse("", "", nil)
